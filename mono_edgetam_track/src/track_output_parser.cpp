...
int TrackOutputParser::RenderSeg(const std::string &img_path,
                     const Parsing &seg, 
...
int32_t TrackOutputParser::Parse(
  ...
  float* low_res = output_tensor->GetTensorData<float>();
...
int32_t TrackOutputParser::UpdateData(std::shared_ptr<DNNTensor> &output_tensor,
  ...
  float* raw_data = output_tensor->GetTensorData<float>();

  std::memcpy(data.data(), raw_data, output_tensor->properties.alignedByteSize);
